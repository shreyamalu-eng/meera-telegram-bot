...
export async function findNews(note) {
  ...
  const query = String(text.search_phrase || "").trim();
  if (!query) return { query, item: null };
  try {
    const results = await googleNews(query);
    return { query, keywords: text.keywords, item: await pickRelevant(note, results) };
  } catch (err) {
    console.warn(`News search failed for "${query}": ${err.message}`);
    return { query, keywords: text.keywords, item: null };
  }
}
