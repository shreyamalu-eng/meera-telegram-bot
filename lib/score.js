...
Also give a short Google News search phrase (2–5 words) built from the note's 3–5 most specific keywords
(ingredients, regulations, claims, industry trends), likely to find a recent relevant news story.`;
...
export async function scoreNote(note) {
  const { text } = await gemini({
    ...
    schema: {
      ...
      required: ["score", "reason", "search_phrase"],
    },
  });
  const score = Math.max(0, Math.min(10, Math.round(Number(text.score) || 0)));
  return {
    score,
    reason: String(text.reason || "").trim(),
    searchPhrase: String(text.search_phrase || "").trim(),
